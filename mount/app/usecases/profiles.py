...
async def get_profile(user_id: int, db: AsyncSession):
	...
	try:
		profile_data: Profile = await profile_repo.get_by_field(
			'user_id', user_id
		)
		...
		return (
			...
			False,
			f'Something went wrong with profile data: {e}',
			None,
		)
...
async def update(user_id: int, profile_data: ProfileUpdate, db: AsyncSession):
	...
	try:
		profile_exists: Profile = await profile_repo.get_by_field(
			'user_id', user_id
		)
		...
		new_profile: Profile = await profile_repo.update(
			profile_exists.id, profile_exists.__dict__.copy()
		)

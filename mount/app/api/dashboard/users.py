...
async def user_status_change(
    ...
    user: StandardResponse = Depends(has_permission("dashboard:users-password:update")),
    db: AsyncSession = Depends(get_db)
    ):
    user_status_code, user_success, user_message, user_data = user
    if not user_success:
    ...
    status_code, success, message, data = await users_usecases.update_password(new_password.new_password, user_id, db, False, None)
    return standard_response(status_code, success, message, data)